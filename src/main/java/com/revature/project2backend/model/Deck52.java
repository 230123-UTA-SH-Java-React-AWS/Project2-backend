...
public class Deck52 {
    ...
    // This returns this Deck52 instance so that a deck instance can be created with
    // this logic:
    ...
    // Shorthand function that can be used to quickly determine if peek() and pop()
    // are safe to execute.
    ...
    public Card52 peek() throws IndexOutOfBoundsException {
        if (deck.size() == 0)
            throw new IndexOutOfBoundsException("There are no more cards left to take.");
    ...
    public Card52 deal() throws IndexOutOfBoundsException {
        if (deck.size() == 0)
            throw new IndexOutOfBoundsException("There are no more cards left to take.");
        return deck.remove(deck.size() - 1); // Index is chosen to avoid many index reassignments each time a card is
                                             // taken
    }
     ...
     * @throws IndexOutOfBoundsException If the number of cards to deal is larger
     *                                   than the amount left in the deck.
     */
    public List<Card52> deal(int number) throws IndexOutOfBoundsException {
        if (number > deck.size())
            throw new IndexOutOfBoundsException("There aren't enough cards left to deal " + number + '!');
